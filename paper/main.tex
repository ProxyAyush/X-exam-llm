...
\end{equation}
where $G$ is the Generator, $C$ is the Cross-Examiner, and $S$ is the Judge's correctness score. The objective was to see if $C$ could force $G$ to become perfectly reliable.

\section{Experimental Results: Adversarial Capitulation}

Our evaluation across 8 datasets revealed a stark degradation in accuracy when deploying the X-Exam adversarial pipeline compared to a zero-shot baseline. 
