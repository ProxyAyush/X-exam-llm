...
class HLEController:
    ...
    def call_api_with_retry(self, model, prompt, system_prompt, retries=3, force_model=False):
        ...
        if limits:
            ...
            if len(self.request_times[model]) >= limits["rpm"]:
                wait_time = 60 - (now - self.request_times[model][0]) + 1
                self.log(f"DEBUG: Rate limit reached for {model}. Waiting {wait_time:.1f}s")
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if response.status_code == 429:
                    self.log(f"WARNING: 429 Rate Limit on attempt {attempt+1}")
                    if self.rotate_key():
                        continue # Try again with new key
                    else:
                        # Sleep and retry if no keys left
                        sleep_time = (2 ** attempt) * 5 + random.random()
